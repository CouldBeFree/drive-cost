<template>
  ...
  >
    <form @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div v-if="vehicles.length > 1" class="group">
...
const resetForm = () => {
  formData.vehicle_id = props.vehicles.length === 1 && props.vehicles[0] ? props.vehicles[0].id : ''
  formData.date = ''
...
watch(() => props.modelValue, (isOpen) => {
  ...
  } else if (isOpen && !props.entry && props.vehicles.length === 1 && props.vehicles[0]) {
    formData.vehicle_id = props.vehicles[0].id
  } else if (!isOpen) {
    resetForm()
  }
})
